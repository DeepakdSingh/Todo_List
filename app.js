...
const intro1 = new TaskMaker({
    name: " <-- Check To delete , To add (+)"
});
const intro2 = new TaskMaker({
    name: " Url+/listname to create new list"
});
...
app.get("/", (req, res)=>{
    TaskMaker.find((err,result)=>{
        if(err){
            ...
            if(result.length === 0){
                TaskMaker.insertMany([intro1,intro2]).then(()=>res.redirect("/"));   
            }else{
